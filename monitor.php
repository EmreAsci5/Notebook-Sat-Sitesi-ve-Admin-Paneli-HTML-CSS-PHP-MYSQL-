...
<div id="TumSayfa">
    ...
    <div id="anaEkran">
        ...
        <?php
        include("baglan.php");
        $monitor=$db->query("SELECT * FROM urunler INNER JOIN urunresimler ON urunresimler.urun_id=urunler.urun_id
        INNER JOIN fiyatlar ON fiyatlar.urun_id=urunler.urun_id
        INNER JOIN model ON model.model_id=urunler.model_id
        INNER JOIN kategori ON kategori.kategori_id=urunler.kategori_id
        WHERE kategori.kategori_id=6;
         ");
        while($kayit=$monitor->fetch())
        {
        ...
        ?>
         ...
         <div id="urun" style="height:600px"><a href="goruntuleme.php?urun_id=<?=$kayit["urun_id"]?>" id="gorunum"> 
           ...
           <div id="urunBilgiler">
            <div id="urunModel"><?=$kayit["model_ad"]." ".$kayit["model_seri"]?> <br><div style="font-size:15px;"> Oyuncu Monitörü</div></div>
            <ul>
           <?php
           ...
           <?php } ?>
            ...
            <div id="sepeteEkle">  <a href="monitor.php?urunid=<?=$kayit["urun_id"]?>" id="sepeteekle">Sepete Ekle</a></div>
           </div>
           </a>
         </div>
  <?php } ?>
