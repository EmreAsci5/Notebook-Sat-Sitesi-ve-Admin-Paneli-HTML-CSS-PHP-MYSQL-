...
<div id="TumSayfa">
    ...
    <div id="anaEkran">
        ...
        <?php
        include("baglan.php");
        $klavye=$db->query("SELECT * FROM urunler INNER JOIN urunresimler ON urunresimler.urun_id=urunler.urun_id
        INNER JOIN fiyatlar ON fiyatlar.urun_id=urunler.urun_id
        INNER JOIN model ON model.model_id=urunler.model_id
        INNER JOIN kategori ON kategori.kategori_id=urunler.kategori_id
        WHERE kategori.kategori_id=5;
         ");
        while($kayit=$klavye->fetch())
        {
        ...
        ?>
         ...
         <div id="urun" style="height:600px"> <a href="goruntuleme.php?urun_id=<?=$kayit["urun_id"]?>" id="gorunum">
           ...
           <div id="urunBilgiler">
            <div id="urunModel"><?=$kayit["model_ad"]." ".$kayit["model_seri"]?> <br><div style="font-size:15px;"> Oyuncu Klavyesi</div></div>
            <ul>
           <?php
           ...
           <?php } ?>
            ...
            <div id="sepeteEkle">  <a href="klavye.php?urunid=<?=$kayit["urun_id"]?>" id="sepeteekle">Sepete Ekle</a></div>
           </div>
           </a>
         </div>
  <?php } ?>
